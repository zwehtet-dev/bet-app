<template>
  <div class="text-white">
      ...
      <div class="bg-gradient-to-r from-blue-500/20 to-cyan-500/20 rounded-xl p-4 border border-blue-500/20">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm font-bold">{{ t('morningSession') }}</p>
            <p class="text-[10px] text-white/40">12:00 PM</p>
          </div>
          <div class="bg-blue-500/20 px-4 py-2 rounded-xl">
            <p class="text-2xl font-black text-blue-400 font-mono">{{ timer }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="px-4 py-2">
      <div class="bg-white/5 rounded-xl p-4 border border-white/5">
        ...
        <div class="flex flex-wrap gap-1.5 min-h-[32px]">
          <span v-for="n in selected" :key="n" class="bg-blue-500 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
            {{ n.toString().padStart(2, '0') }}
            <button @click="toggle(n)" class="text-white/60 hover:text-white">×</button>
          </span>
        ...
        <div v-if="reverseNums.length" class="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span v-for="n in reverseNums.slice(0, 5)" :key="n" class="text-[10px] bg-orange-500/20 text-orange-300 px-1.5 py-0.5 rounded">{{ n.toString().padStart(2, '0') }}</span>
            <span v-if="reverseNums.length > 5" class="text-[10px] text-white/30">+{{ reverseNums.length - 5 }}</span>
          </div>
    ...
    <!-- Patterns -->
    <div class="px-4 py-2">
      <button @click="showPatterns = !showPatterns" class="flex items-center gap-1 text-xs text-white/50 mb-2">
        {{ t('specialPatterns') }}
      ...
      <div v-if="showPatterns" class="flex flex-wrap gap-2">
        <button v-for="p in patterns" :key="p.id" @click="applyPattern(p)" class="bg-white/5 hover:bg-purple-500/20 px-3 py-1.5 rounded-lg text-xs font-medium border border-white/10 active:scale-95">
          {{ p.name }} <span class="text-white/40">({{ p.nums.length }})</span>
        </button>
      </div>
    </div>
    ...
    <div class="px-4 py-2">
      <div class="grid grid-cols-10 gap-1">
        <button v-for="n in 100" :key="n-1" @click="toggle(n-1)" 
                :class="['aspect-square rounded-lg text-[11px] font-bold transition-all active:scale-90', selected.includes(n-1) ? 'bg-blue-500 shadow-lg shadow-blue-500/30' : 'bg-white/5 text-white/60 hover:bg-white/10']">
    ...
    <div class="px-4 py-2 pb-6">
      <div class="bg-white/5 rounded-xl p-4 border border-white/5 mb-4">
        ...
        <div class="flex justify-between text-sm">
          ...
          <span class="font-black text-green-400">{{ formatBalance(total * 85) }} {{ t('mmk') }}</span>
        </div>
      </div>
      <button @click="placeBet" :disabled="!canBet || loading" 
              :class="['w-full py-4 rounded-xl text-sm font-bold transition-all active:scale-[0.98]', canBet && !loading ? 'bg-gradient-to-r from-blue-500 to-cyan-500 shadow-lg shadow-blue-500/25' : 'bg-white/10 text-white/30']">
...
const placeBet = async () => {
  ...
  showToast('2D bet placed!', 'success')
  selected.value = []
  loading.value = false
}
