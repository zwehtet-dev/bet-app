<template>
  <div class="min-h-screen text-white">
    ...
    <!-- Bet Amount Selection (for Maung style) -->
    <div v-if="gameType === 'Maung'" class="px-4 py-2">
      <div class="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-4">
        <div class="flex items-center justify-between mb-3">
          <p class="text-sm">{{ t('betAmount') }}</p>
          <p class="text-sm font-bold">{{ formatBalance(betAmount) }} {{ t('mmk') }}</p>
        </div>
        <div class="flex gap-2 mb-3">
          <button v-for="amount in [1000, 2000, 5000, 10000]" :key="amount"
    ...
    <div class="px-4 py-2">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="loadMatches" :disabled="loading" class="text-sm text-blue-400 hover:text-blue-300">
      ...
      <div v-else class="space-y-3">
        <div v-for="match in matches" :key="match.id" 
          ...
          <div class="space-y-2">
            <!-- Over/Under Options -->
            <div v-if="match.overUnderOptions && match.overUnderOptions.length > 0" class="mb-3">
        ...
        <div class="space-y-2 mb-4">
          <div v-for="bet in selectedBets" :key="`${bet.matchId}-${bet.optionId}`"
            ...
            <div class="flex-1">
              ...
              <div v-if="gameType === 'Body'" class="mt-2">
                <input v-model.number="bet.amount" type="number" min="1000" step="500"
                       class="w-full bg-white/20 text-white rounded px-2 py-1 text-sm"
...
const gameType = ref('Body') // Body or Maung
const betAmount = ref(2000) // For Maung style
const message = ref('')
const messageType = ref('success')

// Demo matches data with enhanced structure
const demoMatches = [
  {
    id: 1,
...
const getBetTeamId = (optionId, match) => {
  switch (optionId) {
    ...
    default: return match.id * 10 + 4 // For over/under
  }
}

const totalBetAmount = computed(() => {
  if (gameType.value === 'Maung') {
...
const potentialWin = computed(() => {
  return selectedBets.value.reduce((sum, bet) => {
    const amount = gameType.value === 'Maung' ? betAmount.value : (bet.amount || 0)
...
const canPlaceBet = computed(() => {
  return selectedBets.value.length > 0 && 
         ...
         isLoggedIn.value &&
         (gameType.value === 'Maung' || selectedBets.value.every(bet => bet.amount > 0))
})
...
const loadMatches = async () => {
  ...
  // await loadSoccerMatches(gameType.value)
}
...
const placeSoccerBet = async () => {
  ...
  try {
    const soccerBetDetails = selectedBets.value.map(bet => ({
      ...
      betType: bet.betType,
      amount: gameType.value === 'Body' ? bet.amount : undefined
    }))
    
    const betData = {
      gameType: gameType.value,
      amount: gameType.value === 'Maung' ? betAmount.value : undefined,
      soccerBetDetails
    }
